...
#if SSC_FORCE_FREERUN
    /*
     * The RT-Thread v2 port is polling-only.  SSC 5.11's generated
     * StartInputHandler() otherwise selects SM synchronous mode whenever
     * PDO outputs exist, even though AL_EVENT_ENABLED is zero.  Without a
     * real LAN9252 AL Event ISR, that leaves PROCESS_OUTPUT_EVENT unhandled
     * and SAFEOP -> OP ends with the SyncManager watchdog.
     */
    SyncType0x1C32 = SYNCTYPE_FREERUN;
    SyncType0x1C33 = SYNCTYPE_FREERUN;
...
{
    extern void ssc_diag_latch_al_status(UINT8 al_status, UINT16 al_code);
    UINT16 Value = alStatusCode;

    ssc_diag_latch_al_status(alStatus, alStatusCode);
